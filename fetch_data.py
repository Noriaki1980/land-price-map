...
# -*- coding: utf-8 -*-
"""
fetch_data.py — 国土数値情報 地価公示データ(L01)の取得と正規化(年次価格系列つき)
...
方針転換の経緯:
  ...
  一度は年次スライダー機能を見送ったが、この横持ち構造をそのまま系列
  として抽出することで、追加ダウンロードなしにスライダー機能を実現できる
  ことが分かったため、価格系列(series)を各地点に持たせる形に再拡張した。

  価格系列の列は名前をハードコードせず、値パターン(v_price_series_candidate)
  で総当たり検出し、列コード昇順(=年の古い順)に1983年起点で年を割り当てる。
  最後の年が--yearと一致するかを整合性チェックとして使う。
...
出力:
  ...
  data/points.json               ... {series_years: [1983,...,2026], points: [{id,lat,lng,use,price,yoy,series},...]}
...
def v_price_series_candidate(values) -> float:
    """
    年次価格系列の候補列: 0(未選定年)を許容する価格らしい列。
    v_priceと違い、ゼロ混入を許すのは「その年に地点がまだ選定されていない」
    ケースが多いため。非ゼロ値の分布で価格らしさを判定する。
    """
    nums = [_num(v) for v in values]
    ok = [n for n in nums if n is not None]
    if len(ok) < len(values) * 0.99 or not ok:
    ...
    if any(n < 0 for n in ok):
        return 0.0
    nonzero = [n for n in ok if n > 0]
    if len(nonzero) < len(ok) * 0.05:  # ほぼ全部ゼロの列は除外
        return 0.0
    med = statistics.median(nonzero)
    if not (500 <= med <= 5_000_000) or max(nonzero) > 200_000_000:
        return 0.0
    return 1.0

def _col_code_num(col: str):
    """列名末尾の数値部分を取り出す(L01_078 -> 78)。ソート用。"""
    m = re.search(r"(\d+)$", col)
    return int(m.group(1)) if m else -1

def _longest_consecutive_run(nums: list[int]) -> list[int]:
    """整数列から、差が1ずつの最長連続区間を1つ返す。系列本体は列コードが
    途切れなく連続しているはずなので、単発の誤検出列(飛び地)を排除できる。"""
    if not nums:
        return []
    nums = sorted(set(nums))
    best_run, cur_run = [nums[0]], [nums[0]]
    for n in nums[1:]:
        if n == cur_run[-1] + 1:
            cur_run.append(n)
        else:
...
def detect_price_series(feats: list[dict], exclude: set, target_year: int) -> dict:
    """
    価格系列候補を全列から総当たりで洗い出し、列コードが途切れなく連続する
    最長区間だけを本物の系列として採用する(単発の誤検出列を自動的に排除)。
    1983年起点で年を割り当て、最後の年が target_year と一致するかを
    整合性チェックとして使う。
    """
    candidates = []
    code_to_col = {}
    for c in feats[0].keys():
        if c.startswith("_") or c in exclude:
            continue
        if v_price_series_candidate(column_values(feats, c)) > 0:
            code = _col_code_num(c)
            candidates.append(code)
            code_to_col[code] = c

    run = _longest_consecutive_run(candidates)
    columns = [code_to_col[code] for code in run]

    n = len(columns)
    start_year = 1983
    years = list(range(start_year, start_year + n))
    consistent = (years[-1] == target_year) if years else False
...
def normalize(feats: list[dict], colmap: dict, series_info: dict) -> list[dict]:
    out = []
    pc = colmap["price"]["col"]
    yc = colmap["yoy"]["col"]
    uc = colmap["use"]["col"]
    ...
    series_cols = series_info["columns"]
    for f in feats:
        price = _num(f.get(pc))
        ...
        # 価格系列: 未選定年は0のままだとグラフ上"価格0円"に見えるためnullにする
        series = []
        for c in series_cols:
            v = _num(f.get(c))
            series.append(int(v) if v and v > 0 else None)
        out.append({
            "id": f"{muni}-{addr}",
...
def validate(rows: list[dict], colmap: dict, series_info: dict) -> list[str]:
    errs = []
    ...
    if not series_info["consistent"]:
        errs.append(
            f"価格系列の年整合性チェックに失敗: 検出した列数={len(series_info['columns'])}件"
            f"(1983年起点で最終年={series_info['years'][-1] if series_info['years'] else 'なし'}"
            f" が対象年と不一致)。mapping_report.jsonのseriesを確認してください。")
    if not rows:
        return errs
    ...
    # 最新年(seriesの最後)とprice列の値が概ね一致するかも確認(整合性の二重チェック)
    if series_info["consistent"] and rows:
        mismatch = sum(1 for r in rows
                       if r["series"] and r["series"][-1] is not None
                       and abs(r["series"][-1] - r["price"]) > max(1, r["price"] * 0.01))
        if mismatch > len(rows) * 0.02:
            errs.append(f"価格系列の最終年とprice列が不一致な地点が多い: {mismatch}件")
    return errs
...
def main():
    ...
    exclude = {colmap[k]["col"] for k in ("use", "muni_code", "address") if colmap[k]["col"]}
    series_info = detect_price_series(feats, exclude, args.year)
    rows = normalize(feats, colmap, series_info)
    errs = validate(rows, colmap, series_info)
    ...
    report = {
        ...
        "columns": colmap,
        "series": {
            "n_columns": len(series_info["columns"]),
            "columns": series_info["columns"],
            "years": series_info["years"],
            "consistent": series_info["consistent"],
        },
        "errors": errs,
    }
    ...
    output = {
        "series_years": series_info["years"],  # [1983, 1984, ..., 2026]
        "points": rows,
    }
    OUT_PATH.write_text(json.dumps(output, ensure_ascii=False, separators=(",", ":")),
                        encoding="utf-8")
    print(f"OK: {len(rows):,} 地点 -> {OUT_PATH} "
          f"(価格系列: {series_info['years'][0]}〜{series_info['years'][-1]}, "
          f"{len(series_info['columns'])}年分)")
